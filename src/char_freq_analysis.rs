...
pub fn calc_char_freq(data: &Vec<u8>) -> HashMap<u8, f32> {
    ...
    // Iterate through bytes, and save frequency of byte as fraction of total
    for &byte in data {
...
pub fn calc_euclidean_dist(freq1: &HashMap<u8, f32>, freq2: &HashMap<u8, f32>) -> f32 {
    ...
    // Calculate euclidean distance for all bytes in freq1
    for (&byte, &freq) in freq1.iter() {
        let diff = freq - freq2.get(&byte).unwrap_or(&0.0);
        sum += diff * diff;
    }

    // Calculate euclidean distance for all bytes in freq2 that are not in freq1
...
pub fn freq_file_to_hashmap(file_path: &str) -> HashMap<u8, f32> {
    ...
    // Iterate through lines, and save frequency of byte as fraction of total
